...
def zoekresultaat():
    ...
    if request.method == "POST":
        ...
        from urllib.request import urlopen
        zoekresultaten = (json.loads(str((requests.get("https://api.themoviedb.org/3/search/movie?api_key=9c226374f10b2dcd656cf7c348ee760a&language=nl&query=" + zoekterm + "&include_adult=false&page=1").content).decode('UTF-8'))))["results"]
        if not zoekresultaten:
            return apology("Geen zoekresultaten")
        else: return render_template("zoekresultaten.html", zoekresultaten=zoekresultaten)
